...
void MeshSetVk::updateObjDescriptionBuffer()
{
  ...
  {
    updateInstanceToObjIndexBuffer();
    return;
  }
...
void MeshSetVk::updateInstanceToObjIndexBuffer()
{
  if(instanceToObjIndexBuffer.buffer != VK_NULL_HANDLE)
    m_alloc->destroyBuffer(instanceToObjIndexBuffer);

  if(instances.empty())
    return;

  std::vector<uint32_t> mapping(instances.size());
  for(uint32_t i = 0; i < instances.size(); ++i)
    mapping[i] = instances[i].objIndex;

  NVVK_CHECK(m_alloc->createBuffer(instanceToObjIndexBuffer, mapping.size() * sizeof(uint32_t),
                                   VK_BUFFER_USAGE_STORAGE_BUFFER_BIT));
  NVVK_DBG_NAME(instanceToObjIndexBuffer.buffer);

  VkCommandBuffer cmdBuf = m_app->createTempCmdBuffer();
  NVVK_CHECK(m_uploader->appendBuffer(instanceToObjIndexBuffer, 0, std::span(mapping)));
  m_uploader->cmdUploadAppended(cmdBuf);
  m_app->submitAndWaitTempCmdBuffer(cmdBuf);
  m_uploader->releaseStaging();
}
...
void MeshSetVk::rtxInitAccelerationStructures()
{
  ...
  if(!instances.empty())
  {
    ...
    for(uint32_t sceneInstanceIndex = 0; sceneInstanceIndex < instances.size(); ++sceneInstanceIndex)
    {
      const Instance& objInst                     = instances[sceneInstanceIndex];
      VkAccelerationStructureInstanceKHR asInst{};
      asInst.transform                            = nvvk::toTransformMatrixKHR(objInst.transform);
      asInst.instanceCustomIndex                  = sceneInstanceIndex;
