...
public:  // Methods specializing IAppElement
  ...
  void setProfilerViewSettings(std::shared_ptr<nvapp::ElementProfiler::ViewSettings> settings);
...
private:
  ...
  void applyUiModeSettings(bool requestDockLayout = false);
  void redockBottomPanels();

  void guiDrawRendererStatisticsWindow(bool* pOpen);
  void guiDrawMemoryStatisticsWindow(bool* pOpen);
  ...
  std::filesystem::path makeDepthOutputFilename() const;
  std::filesystem::path makeSegOutputFilename() const;
  std::filesystem::path makeMultiViewSessionDir() const;
...
private:
  ...
  std::filesystem::path m_depthOutputDirectory{"_outputs/depth"};
  std::filesystem::path m_segOutputDirectory{"_outputs/seg"};
  std::filesystem::path m_multiViewOutputDirectory{"_outputs/multiview"};
