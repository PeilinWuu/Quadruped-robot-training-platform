...
void GaussianSplattingUI::setProfilerViewSettings(std::shared_ptr<nvapp::ElementProfiler::ViewSettings> settings)
{
  m_profilerViewSettings = std::move(settings);
  applyUiModeSettings();
}

void GaussianSplattingUI::applyUiModeSettings(bool requestDockLayout)
{
  const bool debugMode = (m_uiMode == UiMode::Debug);

  if(m_profilerViewSettings)
    m_profilerViewSettings->show = debugMode;

  m_showMemoryStatistics    = debugMode;
  m_showRenderingStatistics = debugMode;

  if(requestDockLayout)
    m_pendingBottomPanelDock = true;
}

void GaussianSplattingUI::redockBottomPanels()
{
  auto undockDebugPanels = []() {
    ImGui::DockBuilderDockWindow("Profiler", 0);
    ImGui::DockBuilderDockWindow("Memory Statistics", 0);
    ImGui::DockBuilderDockWindow("Rendering Statistics", 0);
  };

  ImGuiWindow* anchor = ImGui::FindWindowByName("Export Preview");
  if(anchor == nullptr || anchor->DockId == 0)
  {
    if(m_uiMode == UiMode::Work)
      undockDebugPanels();
    return;
  }

  const ImGuiID nodeId = anchor->DockId;
  ImGui::DockBuilderDockWindow("Export Preview", nodeId);

  if(m_uiMode == UiMode::Debug)
  {
    ImGui::DockBuilderDockWindow("Profiler", nodeId);
    ImGui::DockBuilderDockWindow("Memory Statistics", nodeId);
    ImGui::DockBuilderDockWindow("Rendering Statistics", nodeId);
  }
  else
  {
    undockDebugPanels();
...
void GaussianSplattingUI::onUIMenu()
{
  ...
  if(ImGui::BeginMenu("View"))
  {
    ...
    ImGui::MenuItem(ICON_MS_SPACE_DASHBOARD " Show UI Panels", "", &m_showUI);
    ImGui::Separator();
    if(ImGui::MenuItem("Work UI Mode", nullptr, m_uiMode == UiMode::Work))
    {
      m_uiMode = UiMode::Work;
      applyUiModeSettings(true);
    }
    if(ImGui::MenuItem("Debug UI Mode", nullptr, m_uiMode == UiMode::Debug))
    {
...
void GaussianSplattingUI::onUIRender()
{
  ...
  {
    if(notify == ExportPreviewNotify::MultiView && !lastMultiViewSessionDir().empty())
      m_exportPreview.setLastMultiViewSession(lastMultiViewSessionDir());
    m_exportPreview.notifyExport(notify);
  }
  ...
  if(m_pendingBottomPanelDock)
  {
    redockBottomPanels();
    m_pendingBottomPanelDock = false;
  }

  if(m_uiMode == UiMode::Debug)
  {
    guiDrawRendererStatisticsWindow(&m_showRenderingStatistics);
...
void GaussianSplattingUI::guiDrawRendererProperties()
{
  ...
  if(PE::entry("Depth Output", [&] { return ImGui::Button("Export Depth PNG"); },
               "Export a grayscale depth visualization PNG from the current view (Ctrl+Shift+D)."))
  {
    requestDepthPngExport(makeDepthOutputFilename());
  }

  PE::Text("Depth output folder", m_depthOutputDirectory.string().c_str());

  if(PE::entry("Seg Output", [&] { return ImGui::Button("Export Seg PNG"); },
               "Export instance-level segmentation PNG (Ctrl+Shift+S). ID 0=bg, 1=3DGS, 2+=mesh instances."))
  {
    requestSegPngExport(makeSegOutputFilename());
  }

  PE::Text("Seg output folder", m_segOutputDirectory.string().c_str());

  ImGui::BeginDisabled(isMultiViewExportActive());
  if(PE::entry("Multi-View Output", [&] { return ImGui::Button("Export Multi-View"); },
               "Batch export rgb/depth/seg for all stored camera presets (Ctrl+Shift+M)."))
  {
    requestMultiViewExport(makeMultiViewSessionDir());
  }
  ImGui::EndDisabled();

  if(isMultiViewExportActive())
    PE::Text("Multi-view status", multiViewExportStatus().c_str());

  PE::Text("Multi-view output folder", m_multiViewOutputDirectory.string().c_str());
...
void GaussianSplattingUI::guiDrawRendererStatisticsWindow(bool* pOpen)
{
  if(ImGui::Begin("Rendering Statistics", pOpen))
  {
...
void GaussianSplattingUI::guiDrawMemoryStatisticsWindow(bool* pOpen)
{
  ...
  if(ImGui::Begin("Memory Statistics", pOpen))
  {
...
std::filesystem::path GaussianSplattingUI::makeDepthOutputFilename() const
{
  ...
  oss << "depth_" << std::put_time(&tm, "%Y%m%d_%H%M%S") << "_" << std::setw(3) << std::setfill('0') << ms << ".png";
  return m_depthOutputDirectory / oss.str();
}

std::filesystem::path GaussianSplattingUI::makeSegOutputFilename() const
{
  const auto now = std::chrono::system_clock::now();
  const auto tt  = std::chrono::system_clock::to_time_t(now);
  std::tm     tm{};
  ...
  std::ostringstream oss;
  oss << "seg_" << std::put_time(&tm, "%Y%m%d_%H%M%S") << "_" << std::setw(3) << std::setfill('0') << ms << ".png";
  return m_segOutputDirectory / oss.str();
}

std::filesystem::path GaussianSplattingUI::makeMultiViewSessionDir() const
{
  const auto now = std::chrono::system_clock::now();
  const auto tt  = std::chrono::system_clock::to_time_t(now);
  std::tm     tm{};
  ...
  return m_multiViewOutputDirectory / oss.str();
}
