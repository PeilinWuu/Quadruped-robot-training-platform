...
class StdoutWriter {
 public:
  ...
  void pose(std::string line) { std::lock_guard lock(mutex_); latest_pose_ = std::move(line); condition_.notify_one(); }
  void stop() {
 ...
 private:
  void run() {
    ...
    for (;;) {
      condition_.wait(lock, [this] { return stopping_ || !responses_.empty() || latest_pose_.has_value(); });
      if (stopping_ && responses_.empty() && !latest_pose_) break;
      std::string line;
      if (!responses_.empty()) { line = std::move(responses_.front()); responses_.pop_front(); }
      else { line = std::move(*latest_pose_); latest_pose_.reset(); }
      lock.unlock(); std::cout << line << '\n' << std::flush; lock.lock();
    }
  }
  std::mutex mutex_; std::condition_variable condition_; std::deque<std::string> responses_;
  std::optional<std::string> latest_pose_; bool stopping_{false}; std::thread thread_;
};
}
...
int main(int argc, char** argv) {
  ...
  StdoutWriter writer;
  sidecar::ProtocolHandler handler(root.string(), [&writer](std::string event) { writer.pose(std::move(event)); });
  std::string line;
  for (;;) {
