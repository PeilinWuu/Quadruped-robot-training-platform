...
const metrics=[];
await page.waitForFunction(()=>runtime.fireVolume.proxy.active);
const shared=await page.evaluate(()=>[...runtime.fireVolume.companionVolumes.values()].every(v=>v.proxy===runtime.fireVolume.proxy));if(!shared)throw Error('Depth not shared');
for(const id of ['table_high','sofa_high','curtain_high']){
 ...
 await page.evaluate(()=>{fire.pause();fire.presentationSeconds=24;fire.atmosphereEnabled=false;fire.depthOcclusion=false});await page.waitForTimeout(150);
 await page.screenshot({path:out+'/'+id+'-before.png'});
 await page.evaluate(()=>{fire.depthOcclusion=true});await page.waitForFunction(()=>runtime.fireVolume.proxy.active);await page.waitForTimeout(150);
...
const atmosphereTime=await page.evaluate(()=>{fire.pause();return fire.presentationSeconds});await page.waitForTimeout(200);
if(await page.evaluate(t=>fire.presentationSeconds!==t,atmosphereTime))throw Error('Atmosphere did not pause');
...
const cleanup=await page.evaluate(()=>({services:fire.getCompanions().size,volumes:runtime.fireVolume.companionVolumes.size,stillPlaying:fire.getState().playing,atmosphereDisposed:runtime.fireVolume.atmosphere===null}));
...
await browser.close();if(errors.length||!pauseCheck||!fp||cleanup.services||cleanup.volumes||!cleanup.atmosphereDisposed)throw Error('Room fire test failed');
