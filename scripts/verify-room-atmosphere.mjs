...
const out=process.env.FIRE_QA_OUTPUT ?? 'tmp/room-atmosphere-results'; await fs.mkdir(out,{recursive:true});
...
for(const id of ['table_high','sofa_high','curtain_high']){
 ...
 await page.evaluate(()=>{fire.pause();fire.presentationSeconds=24;fire.atmosphereEnabled=false});await page.waitForTimeout(150);
 await page.screenshot({path:out+'/'+id+'-before.png'});
 await page.evaluate(()=>{fire.atmosphereEnabled=true});await page.waitForFunction(()=>runtime.fireVolume.atmosphere.entity.enabled);await page.waitForTimeout(150);
