...
export const tauriSimulationAdapter: SimulationAdapter = {
  ...
  loadModel: (modelId, environmentId = 'flat-ground-v1') => command<ModelMetadata>('simulation_load_model', { modelId, environmentId }),
  listAvailableEnvironments: () => command('simulation_list_environments'),
  getCurrentEnvironment: () => command('simulation_current_environment'),
  getLatestCollisionState: () => command('simulation_latest_collision'),
  getLatestCollisionEvent: () => command('simulation_latest_collision_event'),
