...
interface AppState {
  ...
  setStatus: (status: DashboardSimulationStatus) => void; setSpeed: (speed: number) => void; setSensor: (sensor: string) => void; setControlMode: (mode: ControlMode) => void; tick: () => void; appendMockMetrics: () => void
  ...
  return {
    ...
    initialize: async () => {
      const [sceneResult, robotResult, sensorResult, taskResult, metricsResult] = await Promise.all([
        ...
        services.training.getTask(),
        MOCK_RESEARCH_CHARTS.enabled
          ? services.training.getMetrics()
          : Promise.resolve({ data: [], source: 'mock' as const }),
      ])
    ...
    appendMockMetrics: () => set((state) => {
      if (!MOCK_RESEARCH_CHARTS.metricsProducerEnabled) return {}
