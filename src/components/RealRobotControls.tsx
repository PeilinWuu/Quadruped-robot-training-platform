...
import type { RobotSpatialState, SpatialOriginAlignment } from '../services/spatial/types'
import { composeTransforms, yawFromQuaternion } from '../services/spatial/transformMath'
import { applySpatialOriginAlignment, createSpatialOriginAlignment, spatialAlignmentError, spatialStateFromRealTelemetry } from '../services/spatial/realSpatialAdapter'

const INITIAL_MOVE: RealMoveCommand = { forwardVelocity: 0.05, lateralVelocity: 0, yawRate: 0, durationMs: 500 }
export function RealRobotControls({ initialStatus, referenceSpatialState = null }: { initialStatus?: RealRobotStatus; referenceSpatialState?: RobotSpatialState | null }) {
  ...
  const [originAlignment, setOriginAlignment] = useState<SpatialOriginAlignment | null>(null)
  useEffect(() => {
  ...
  const realSpatial = useMemo(() => spatialStateFromRealTelemetry(status.telemetry, status.telemetryAgeMs), [status.telemetry, status.telemetryAgeMs])
  const alignedRealSpatial = realSpatial && originAlignment ? applySpatialOriginAlignment(realSpatial, originAlignment) : realSpatial
  const alignmentError = alignedRealSpatial && referenceSpatialState && originAlignment
    ? spatialAlignmentError(alignedRealSpatial, referenceSpatialState) : null
  const realWorldBase = alignedRealSpatial
    ? composeTransforms(alignedRealSpatial.worldToOdom.transform, alignedRealSpatial.odomToBase.transform) : null
  const referenceWorldBase = referenceSpatialState
    ? composeTransforms(referenceSpatialState.worldToOdom.transform, referenceSpatialState.odomToBase.transform) : null
  return <section className={`robot-unavailable real-robot-controls ${status.controlEnabled ? 'real-robot-controls--armed' : ''}`}>
    ...
    <section className="real-spatial-alignment">
      <h4>真机/仿真统一坐标</h4>
      <div className="real-robot-actions">
        <button type="button" disabled={!status.robotOnline || !realSpatial || !referenceSpatialState} onClick={() => {
          if (realSpatial && referenceSpatialState) setOriginAlignment(createSpatialOriginAlignment(realSpatial, referenceSpatialState))
        }}>以当前仿真位姿设置同步原点</button>
        <button type="button" disabled={!originAlignment} onClick={() => setOriginAlignment(null)}>清除同步原点</button>
      </div>
      {realWorldBase ? <dl className="robot-performance real-telemetry">
        <div><dt>真机 world XYZ</dt><dd>{realWorldBase.translation.map((value) => value.toFixed(3)).join(' / ')}</dd></div>
        <div><dt>真机 world yaw</dt><dd>{yawFromQuaternion(realWorldBase.rotation).toFixed(3)} rad</dd></div>
        {referenceWorldBase && <><div><dt>仿真 world XYZ</dt><dd>{referenceWorldBase.translation.map((value) => value.toFixed(3)).join(' / ')}</dd></div><div><dt>仿真 world yaw</dt><dd>{yawFromQuaternion(referenceWorldBase.rotation).toFixed(3)} rad</dd></div></>}
        <div><dt>坐标来源</dt><dd>Sport position + IMU RPY · low confidence</dd></div>
        <div><dt>同步原点</dt><dd>{originAlignment ? `已设置 · sim seq ${originAlignment.referenceSequence}` : '未设置（真机局部 odom）'}</dd></div>
        {alignmentError && <><div><dt>位置误差</dt><dd>{alignmentError.distance.toFixed(3)} m</dd></div><div><dt>Yaw 误差</dt><dd>{alignmentError.yaw.toFixed(3)} rad</dd></div></>}
      </dl> : <p>等待同时收到 Sport position 和 LowState IMU RPY。</p>}
      {!referenceSpatialState && <p>请先启动 MuJoCo 仿真，生成同步参考位姿。</p>}
      <p>同步只校准 XYZ 和 yaw，不会用当前 roll/pitch 倾斜世界坐标；Sport 局部位置的漂移仍需真机实测。</p>
    </section>
    ...
    <div className={`real-keyboard ${canAct ? 'real-keyboard--active' : ''}`}>
      <p><b>{canAct ? '真机已加入统一键盘目标' : '真机未加入统一键盘目标'}</b></p>
      <p>W/S 前后 · A/D 横移 · Q/E 旋转 · Space 停止 · Esc 停止并解除。输入框聚焦时不响应。</p>
      <p>请使用仿真区域唯一的“启用同步键盘控制”按钮。真机在线且控制已解锁时，同一运动意图会同时发往真机和仿真。</p>
      <p>中速：vx/vy ±0.30 m/s · yaw ±0.50 rad/s；低速：vx/vy ±0.15 m/s · yaw ±0.25 rad/s。</p>
      <p>真机保持期间每 250 ms 刷新一次高层 Move；真机离线或未解锁时，键盘只控制仿真。</p>
    </div>
    <p>统一键盘组合变化时立即更新两个目标；松开全部运动键或 STOP 后，仿真清零且真机发送三次 API 1003。定时 Move 仍保持有界时长。</p>
