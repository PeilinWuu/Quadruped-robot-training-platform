...
export function SceneSidebar({ notify }: { notify: (text: string) => void }) {
  const { scenes, activeSceneId, selectScene, updateEnvironment } = useAppStore(useShallow((state) => ({ scenes: state.scenes, activeSceneId: state.activeSceneId, selectScene: state.selectScene, updateEnvironment: state.updateEnvironment }))); const active = scenes.find((s) => s.id === activeSceneId)
